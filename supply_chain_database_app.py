"""
================================================================================
科技巨頭台灣供應鏈情報庫 - 完整獨立資料庫程式 (Streamlit 旗艦升級版)
整合 V25.2 DataEngine & WhaleEngine 核心：
1. 移除 st.cache_data 快取阻斷，改採 st.session_state 即時響應，保證資料更新立即生效！
2. 在「每間公司卡片旁」新增獨立的「🔄 更新行情」按鈕，以及「🐋 V25.2 分析」按鈕（置於更新按鈕下方）。
3. 支援雙向對接你的獨立 V25.2 Streamlit 網頁：
   - 內建 V25.2 量化分析引擎：直接在卡片下方展開魚頭分數、撤退風險、實戰防守價(ATR)、主力成本(VWAP60)、健康等級。
   - 跨網頁快速跳轉：可在側邊欄填入你的獨立 V25.2 網址，一鍵帶參數直達外部獨立網頁！
4. 抓取最新股價後，自動連動重新計算：
   - 即時動態本益比 (PE = 股價 / 近四季EPS)
   - 距離法人目標價潛在空間 %
   - 記錄精確更新時間戳記 (last_synced_at)
5. 內建 V25.2 擬真隨機安全延遲 (1.5 ~ 3.2 秒)，徹底防止被認出為爬蟲程式。
================================================================================
"""
...
DB_PATH = "master_supply_chain_db.json"

# ==============================================================================
# 資料庫載入與 Session State 管理（徹底移除快取，確保數據即時刷新）
# ==============================================================================
def init_database():
...
class V25MarketSyncEngine:
    ...
    def fetch_single_stock_price(self, stock_id, status_placeholder=None, apply_delay=True):
        ...
        # 3. 方案 B：台灣證交所/櫃買中心官方 MIS API 備援 (免 Token、免登入、極速穩定)
        try:
...
def apply_vendor_market_update(code, res):
    """將抓到的最新股價寫入 session_state，並自動重新計算動態本益比與目標價空間"""
...
def main():
    st.markdown("""
        <div style="padding: 16px 20px; background: linear-gradient(135deg, #0f172a, #1e1b4b); border-radius: 16px; border: 1px solid rgba(56,189,248,0.25); margin-bottom: 24px;">
            <h1 style="color: #ffffff; margin: 0; font-size: 2rem;">🌐 科技巨頭台灣供應鏈情報庫</h1>
    ...
    with st.sidebar:
        ...
        st.caption("💡 內建 V25.2 擬真延遲 (1.5~3.0秒)，模擬真人看盤防封鎖。")
        ...
        if st.button("🚀 啟動 V25.2 批次擬真安全同步", use_container_width=True):
            ...
            status_box.success(f"✅ 成功完成 {success_count} 檔個股最新行情與動態本益比更新！")
            time.sleep(1)
            st.rerun()
    ...
    with tabs[0]:
        st.markdown("""
            <div style="padding: 14px 18px; background: rgba(12, 22, 42, 0.85); border: 1.5px solid rgba(56, 189, 248, 0.4); border-radius: 14px; margin-bottom: 20px;">
                ...
                <p style="margin: 4px 0 0 0; color: #94a3b8; font-size: 0.88rem;">點選任一國際大廠，直接穿透檢視其在台專屬供應鏈陣列（依上、中、下游劃分）</p>
            </div>
        """, unsafe_allow_html=True)
        ...
        for i, (cid, cinfo) in enumerate(clients.items()):
            ...
            with col:
                ...
                if st.button(f"檢視 {cinfo['name']} 供應鏈", key=f"btn_c_{cid}"):
                    st.session_state["selected_client"] = cid

        if "selected_client" in st.session_state:
            sel_id = st.session_state["selected_client"]
            c = clients[sel_id]
            st.markdown(f"### 🎯 {c['icon']} {c['name']} 在台專屬供應鏈全景")
            st.write(f"**核心採購範疇**：{c['key_focus']}")
            st.write(f"**資本支出目的**：{c.get('capex_purpose', '-')}")
    ...
    with tabs[1]:
        st.markdown("""
            <div style="padding: 14px 18px; background: rgba(26, 17, 46, 0.85); border: 1.5px solid rgba(192, 132, 252, 0.4); border-radius: 14px; margin-bottom: 20px;">
                ...
                <p style="margin: 4px 0 0 0; color: #94a3b8; font-size: 0.88rem;">以產業大趨勢切入，檢視完整上中下游結構與上市櫃關鍵下包商</p>
            </div>
        """, unsafe_allow_html=True)
        ...
        for i, (did, dinfo) in enumerate(domains.items()):
            ...
            with col:
                ...
                st.write(dinfo['desc'])
                if st.button(f"進入 {dinfo['name']}", key=f"btn_d_{did}"):
                    st.session_state["selected_domain"] = did

        if "selected_domain" in st.session_state:
            d_id = st.session_state["selected_domain"]
            d = domains[d_id]
            st.markdown(f"## {d['icon']} {d['name']} 樹狀結構")
    ...
    with tabs[2]:
        ...
        for code, v in vendors.items():
            df_list.append({
                ...
                "毛利率": v.get("margin", "-"),
                "最後更新": v.get("last_synced_at", "初始資料")
            })
...
def render_vendor_card_with_sync(code, v):
    ...
    with card_container:
        col_main, col_btn = st.columns([3.7, 1.3])
        
        with col_main:
            st.markdown(f"""
                <div style="padding: 10px 14px; border: 1px solid rgba(255,255,255,0.12); border-radius: 10px; background: rgba(255,255,255,0.02);">
                    ...
                    <div style="display: flex; justify-content: space-between; font-size: 0.85rem; color: #94a3b8; margin-top: 6px;">
                        <span>最新股價: <strong style="color: #38bdf8; font-size: 1rem;">{v.get('price', '-')}</strong></span>
                        <span>動態本益比: <strong style="color: #34d399; font-size: 1rem;">{v.get('trailing_pe', '-')}</strong></span>
                    </div>
                </div>
            """, unsafe_allow_html=True)
        ...
        with col_btn:
            ...
            if st.button("🔄 更新行情", key=f"btn_sync_{code}", use_container_width=True):
                ...
                with st.spinner(f"正在連網更新 {v['name']} ({code}) ..."):
                    res, err = engine.fetch_single_stock_price(code, apply_delay=False)
                    if res and res.get("status") == "success":
                        apply_vendor_market_update(code, res)
                        st.success(f"✅ {v['name']} 已更新！最新價: {res['price']}")
                        time.sleep(0.5)
                        st.rerun()
                    else:
                        st.error(f"❌ 更新失敗: {err}")

            # 按鈕 2：依照使用者需求，放在同步更新按鈕下方！
        ...
        with st.expander(f"🔍 檢視 {v['name']} 完整基本面情報檔案"):
            ...
            if "last_synced_at" in v:
                st.caption(f"🕒 數據最後更新時間：{v['last_synced_at']}")
...
def render_v25_analysis_panel(code, v):
    ...
    st.markdown(f"""
        <div style="padding: 14px; background: rgba(30, 27, 75, 0.5); border: 1.5px solid #818cf8; border-radius: 12px; margin: 10px 0 16px 0;">
    ...
    # 1. 如果有設定外部獨立 V25.2 Streamlit 網址，提供一鍵跳轉
    v25_url = st.session_state.get("v25_ext_url", "")
    if v25_url:
        target_link = f"{v25_url}/?stock={code}"
        st.link_button(f"🌐 直接在獨立 V25.2 網頁開啟 {v['name']} ({code}) ➔", target_link, use_container_width=True)
